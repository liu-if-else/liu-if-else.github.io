...
**Fast Fourier Transform**: FFT can used to transform signals.  
...
**samples**: The array is the return value of the funciton. The value of any element of the array is the strength of the audio source at a certain frequency. To be optimized with the Fourier transform algorithm, the length of the samples array must be the power of two, minimal 64 and maximum 8192.  
**channel**: Stereo seperates a audio source into different channels and provided them to different audio hardwares to play. In this case, the channel parameter can be used to choose a certain channel to sample by FFT. Setting to 0 will sample all channels.  
**window**: A helper function to the FFT. There is a trade-off of the complexity of the window-function for reducing the noises.  
**usage**:
```csharp
...
The reciprocal of T, $f_s/N$, is called Frequency Resolution. The pictures below are the results of the FFT of the audio. They show that the higher the Frequency Resolution the denser the datas.
...
The length of the spectrumData array is actually the Frequency Resolution and every array's element represents a certain frequency. Unfortunately, we can't get the frequency value of the array's element(but well the strength) at runtime and I can't find any clue in the Unity documentation. So let's try to figure it out through the tests.

Using a software to analyze "[MV] FIESTAR(피에스타) _ Mirror.mp3", we can see that there is a free fall of the strenth at the 16000Hz frequency. 
...
In addtion to the tests, other clues can be found from the angle of the Fourier Transform. The core algorithm of GetSpectrumData function is FFT. As mentioned in the beginning, sound is a sort of wave where frequency decided pitch and amplitude determined volume. 
...
The picture above shows two pairs of sound wave with the same frequency and different amplitudes in a x-time/y-amplitude plane. According to the superposition principle, we can simply add these waves to generate a new one.
...
This is exactly what happend when the different sound waves met in the air and created a mixed tone. The datas of a audio file are just the discrete samples of a complex wave. From this perspective, we can say that making audio/music is adding different sound waves into a complex wave. Just the opposite, the Fourier Transform breaks down complex wave. No matter how complex the wave is, through a series of magical calculations, the FT can identify every unmixed wave within it. 

From the viewpoint of two-dimensional space, the Fourier Transform will translate the complex wave from the "x-time/y-amplitude" plane to the "x-frequency/y-strength" and then to many "x-time/y-strength of a wave with fixed frequency". The strength of every single wave on the time axis will become the value of the correspond element of spectrumData array.

In this transfomation process, FFT firstly needs to sample the complex wave on the time axis with a fast interval. To correctly reconstruct the single-frequency wave, sufficient samples are necessary. Here comes "the Nyquist–Shannon sampling theorem". To put it simply, the theorem states that the number of the sufficient samples is the frequency multiply by two. For instance, for a 3Hz wave you need 6 samples. 

The nowadays music industry adopts the 44100Hz sampling rate standard. So a second length of a audio file contains 44100 samples. Because of the restriction of the Nyquist theorem, the highest frequency FFT can sample is 22050Hz, which is consistent with the conclusion of the test above. Given the highest frequency and the length of the spectrumData array, we can further determine the exact frequencies presented by the elements of the array. For example, the demo project has used a 8192 array:

spectrumData[0]		<=>		22050Hz/8192\*1    = 2.6916 Hz  
spectrumData[1]		<=>		22050Hz/8192\*2    = 5.3833 Hz  
...  
spectrumData[8191]	<=>		22050Hz/8192\*8192 = 22050  Hz  

Adjusting the length of the array, we can obtain any certain frequency and expand the analyze to pitch.
...
According to this pitch/frequncy table, the highest pitch's frequncy of octave 9 is 15804Hz, which explained the free fall at 16000Hz of the FFT result's chart above as well.
...
In the demo project, I obtained the spectrum datas in Update and injected them to the scale of the cubes and the colors of their materials. The workload is distributed to both CPU and GPU. If you want to reduce the burden of CPU, another idea is to translate the whole spectrum datas to the GPU(since you have to use this bandwidth anyway) and to handle all the work to the shaders, like vertex animation, color calculation and etc. Since the highest frequncy of the song is below 16000Hz, the audio visualizer only uses the 5461 of 8192 cubes to construct the itself.

The core script is the Controller: 
